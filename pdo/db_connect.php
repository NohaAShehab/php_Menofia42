...
//    $name = "nayara";
//    $email = "nayera@gmail.com";
//    $inst_stmt = $db->prepare($insert_query);
//    $inst_stmt->bindParam(1, $name);
//    $inst_stmt->bindParam(2, $name);
//    $inst_stmt->execute();
//    echo $db->lastInsertId();
//    echo $inst_stmt->rowCount();
//    var_dump( $db->errorInfo());

    $insert_query= "insert into student(`name`, `email`) values(:username,:useremail)";
    $name = "nayara";
    $email = "nayera@gmail.com";
    $inst_stmt = $db->prepare($insert_query);
    $inst_stmt->bindParam(":useremail", $email, PDO::PARAM_STR);
    $inst_stmt->bindValue(":username", 'noha');
    $inst_stmt->execute();
    echo $db->lastInsertId();
    echo $inst_stmt->rowCount();
    var_dump( $db->errorInfo());
